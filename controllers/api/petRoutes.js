...
router.get('/types/:id', async (req, res) => {
  try {
    ...
    if (petType != null) {
      ...
      res.status(404).json({ message: `No petType found with id: ${req.params.id}` });
    }    
...
router.put('/types/:id', async (req, res) => {
  try {
    const [affectedRowCount] = await PetType.update(req.body, { where: { id: req.params.id } });
...
router.delete('/types/:id', async (req, res) => {
  try {
    const isDeleted = await PetType.destroy({ where: { id: req.params.id } });
